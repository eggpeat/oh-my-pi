...
const ALIBABA_TOKEN_PLAN_EFFORT_COMPAT: OpenAICompat = {
...
export const ALIBABA_TOKEN_PLAN_STATIC_MODELS: readonly ModelSpec<"openai-completions">[] = [
	{
		...
		compat: ALIBABA_TOKEN_PLAN_EFFORT_COMPAT,
	},
	{
		id: "qwen3.7-max",
		name: "Qwen3.7 Max",
...
function alibabaTokenPlanModelCacheProviderId(apiKey: string | undefined, baseUrl: string): string {
	// The native model list is entitlement-scoped; isolate authoritative rows by credential and endpoint.
	const scope = `${apiKey ?? ""}\u0000${baseUrl}`;
	return `alibaba-token-plan:models-v1:${Bun.hash(scope).toString(36)}`;
}
...
export function alibabaTokenPlanModelManagerOptions(
	...
	return {
		...
		cacheProviderId: alibabaTokenPlanModelCacheProviderId(apiKey, baseUrl),
		dynamicModelsAuthoritative: true,
